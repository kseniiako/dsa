...
3. [Time-Based Key-Value Storage](https://github.com/brasssmonkey/dsa/blob/main/timeBasedKeyValueStore.py): a data structure to store and retrieve values that remembers when each value was added. Get method retrieves value that was current at a given time; its implementation relies on binary search.
